...
- [19 — SOLID prinsiplari (refactoring katalar)](./19-solid.md) — SRP/OCP/LSP/ISP/DIP har biri uchun yomon kod → nega yomon → refactoring kata; code smells (god class, feature envy, primitive obsession, data clumps); anemic vs rich domen model; over-engineering tanqidi.
- [20 — Design patterns (GoF) idiomatik PHP 8.4](./20-design-patterns.md) — creational (Factory/Builder/Singleton tuzoqlari), structural (Adapter/Decorator/Proxy/Facade/Composite), behavioral (Strategy/Observer/Command/State/Chain); zamonaviy PHP idiomlari (closure-as-Strategy, `__invoke`, enum-as-State) va pattern abuse tanqidi.
- [21 — Taktik dizayn: Repository, Service, DTO, Value Object](./21-taktik-dizayn.md) — Repository (domen interfeysi / infra implementatsiya), Data Mapper vs Active Record, Service/use-case qatlami, DTO vs Value Object farqi va N+1 muammosi.
