#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""AI/LLM (Python) kitobi markaziy QA: kirill, BOM, font<13, cross-link, SVG XML, rasm, nav, apostrof."""
import os, re, glob
import xml.etree.ElementTree as ET

BOOK = r"C:/Users/imomn/Desktop/e-books/docs/ai-python"
RASM = os.path.join(BOOK, "rasmlar")
CYR = re.compile(r"[Ѐ-ӿ]")          # Cyrillic block (homoglyphs)
TYPO_APOS = re.compile(r"[‘’ʻʼ]")  # typographic apostrophes
EXPECTED_CH = [f"{n:02d}" for n in range(1, 29)]

issues = {"cyrillic": [], "bom": [], "font": [], "img_missing": [], "xlink_missing": [],
          "xml": [], "svg_header": [], "nav": [], "apostrophe": []}

def scan_text(path, text, key):
    for i, line in enumerate(text.splitlines(), 1):
        for m in CYR.finditer(line):
            ch = m.group()
            issues["cyrillic"].append(f"{os.path.basename(path)}:{i} '{ch}'(U+{ord(ch):04X}) ...{line[max(0,m.start()-14):m.start()+14]}...")
        for m in TYPO_APOS.finditer(line):
            if key == "md":
                issues["apostrophe"].append(f"{os.path.basename(path)}:{i} U+{ord(m.group()):04X} ...{line[max(0,m.start()-10):m.start()+10]}...")
...
for f in ch_files:
    ...
    if prev_exp and (not old or old[0] != prev_exp):
        issues["nav"].append(f"{name}: OLDINGI '{old[0] if old else 'YOQ'}' != '{prev_exp}'")
    ...
    if next_exp and (not nxt or nxt[0] != next_exp):
        issues["nav"].append(f"{name}: KEYINGI '{nxt[0] if nxt else 'YOQ'}' != '{next_exp}'")
...
print("="*68)
total = 0
labels = {"cyrillic":"KIRILL","apostrophe":"TIPO-APOSTROF","bom":"BOM","font":"FONT<13",
          "img_missing":"RASM YO'Q","xlink_missing":"CROSS-LINK/BOB","xml":"SVG XML",
          "svg_header":"SVG HEADER","nav":"NAVIGATSIYA"}
